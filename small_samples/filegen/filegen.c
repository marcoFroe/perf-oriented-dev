...
void create_files(const char *dirname, int num_files, int min_file_size,
                  int max_file_size) {
  char full_dirname[MAX_FILENAME_LENGTH];
  snprintf(full_dirname, MAX_FILENAME_LENGTH, "/tmp/generated_cb761228/%s",
           dirname);
...
int main(int argc, char **argv) {
  ...
  default:
    fprintf(stderr,
            "Usage: %s <num_directories> <num_files_per_directory> "
            "<min_file_size> <max_file_size> [<seed>]\n",
            argv[0]);
    return 1;
  }
  ...
  mkdir("/tmp/generated_cb761228", 0755); // Create the parent directory
