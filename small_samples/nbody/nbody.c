...
triple triple_zero() { return (position){0.0, 0.0, 0.0}; }

triple triple_rand() {
  return (position){rand_val(-SPACE_SIZE, SPACE_SIZE),
                    rand_val(-SPACE_SIZE, SPACE_SIZE),
                    rand_val(-SPACE_SIZE, SPACE_SIZE)};
}

void triple_print(triple t) { printf("(%f,%f,%f)", t.x, t.y, t.z); }
...
#define EQ(T1, T2)                                                             \
  (abs((T1).x - (T2).x) < eps && abs((T1).y - (T2).y) < eps &&                 \
   abs((T1).z - (T2).z) < eps)
