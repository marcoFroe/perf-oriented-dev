...
template <size_t StorageSize> SystemLinkedList<StorageSize>::SystemLinkedList() {}

template <size_t StorageSize> SystemLinkedList<StorageSize>::~SystemLinkedList() {}

template <size_t StorageSize>
void* SystemLinkedList<StorageSize>::insert(size_t index, void* value) {
	if(value == nullptr) {
		return nullptr;
	}

	Node* new_node = new Node();
	std::memcpy(new_node->value, value, StorageSize);
...
template <size_t StorageSize> void SystemLinkedList<StorageSize>::remove(size_t index) {
	if(index == 0) {
...
template <size_t StorageSize> void* SystemLinkedList<StorageSize>::read(size_t index) {
...
template <size_t StorageSize> void SystemLinkedList<StorageSize>::write(size_t index, void* value) {
	auto it = this->list.begin();
	size_t current_index = 0;
	...
	if(it != this->list.end()) {
		std::memcpy(it->value, value, StorageSize);
	}
}
