#!/usr/bin/env python3
"""Run benchmarker for `linked` with num_elements=1_000_000 across allocation/iteration combos.
Writes results to `sheet_10/results/linked_1e6_results.csv` and does not overwrite other files.
"""
import subprocess
import csv
import os
...
def main():
    ...
    csv_path = os.path.join(out_dir, 'linked_1e6_results.csv')
            ...
            for num_elements in num_element_values:
                ...
                for allocation in allocation_values:
                    for iteration_type in iteration_types:
                        cmd = ['./benchmarker', str(rw_mix), str(num_elements), str(element_size), 'linked', allocation, iteration_type]
